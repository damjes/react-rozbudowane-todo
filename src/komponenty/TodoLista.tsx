...
function TodoLista(props: {zadania: Zadanie[], zmieniarka: Zmieniarka<Zadanie[]>}) {
	function mutatorPojedynczegoZadania(stareZadanie: Zadanie) {
		return function(mutator: MutatorStanu<Zadanie | undefined>) {
			...
			if(noweZadanie) {
				props.zmieniarka(lista => lista.map(zadanie => stareZadanie.id === zadanie.id ? noweZadanie : zadanie))
			} else {
				props.zmieniarka(lista => lista.filter(zadanie => zadanie.id !== stareZadanie.id))
			}
		} 
	} 
